...
prediction1  = tf.constant([[[0.2,0.7],[0.1,0.9],[0.1,0.9]]], dtype=tf.float32)
...
def sequence_loss2(logits,
  ...
  if len(weights.get_shape()) != 2:
    raise ValueError("Weights must be a [batch_size x sequence_length] "
                     "tensor")
  with ops.name_scope(name, "sequence_loss2", [logits, targets, weights]):
...
#sequence_loss = tf.contrib.seq2seq.sequence_loss(logits=prediction1, targets=y_data, weights = weight)
sequence_loss = sequence_loss2(logits=prediction1, targets=y_data, weights = weight)
